console.log('%c    PoliPhaser - Version 0.2    ', 'background: #728FA5; color: white; font-size:20px;');
...
PP.assets.image.add = function(scene, image, x, y, pivot_x, pivot_y) {
    ...
    return {ph_obj: temp_image, orig_image : image, geometry : new __PP_internal_getset_obj_geometry(temp_image)};
}
...
PP.assets.sprite.add = function(scene, sprite, x, y, pivot_x, pivot_y) {
    ...
    return {ph_obj: temp_image, orig_sprite : sprite, list_of_animations : [], geometry : new __PP_internal_getset_obj_geometry(temp_image)};
}
...
PP.assets.tilesprite.add = function(scene, image, x, y, width, height, pivot_x, pivot_y) {
    ...
    return {ph_obj: temp_ts, orig_image : image, geometry : new __PP_internal_getset_obj_geometry(temp_ts),
 ...
 * @param {string}   background_color   The 6-digits HEX representation of the background color in the format 0xABCDEF.
 * 

 * @return A shape object representing the text.
 */
PP.shapes.text_styled_add = function(scene, x, y, text, fontSize, fontFamily, fontStyle, color, background_color){
    PP.debug.assert(typeof scene === "object", "Parameter error: scene should be a scene object.");
    PP.debug.assert(typeof x === "number", "Parameter error: x should be a number.");
    PP.debug.assert(typeof y === "number", "Parameter error: y should be a number.");
    ...
    let colbkg = "#" + background_color.substring(2).toLowerCase();
 ...
 */
PP.shapes.changeText = function(text_instance, newtext){
 ...
 * @param {type_of_event}    string    Identify the type of interaction. It can be: "pointerdown",
 *                                     "pointerdownoutside", "pointermove", "pointerout",
 *                                     "pointerover", "pointerup", "pointerupoutside", "wheel"
 * @param {function_to_call} function  Callback function to call when an event occurred. The
 ...
 */
PP.camera.start_follow = function(s, obj, offset_x, offset_y) {
    PP.debug.assert(typeof obj === "object",               "Parameter error: obj should be an object.");
    PP.debug.assert(typeof obj.ph_obj === "object",        "Parameter error: obj is an object but it is not a valid object.");
    ...
    s.cameras.main.startFollow(obj.ph_obj);
    s.cameras.main.setFollowOffset(offset_x, offset_y);
}
 ...
 * @param {number} delay         the amount of seconds to be waited
 * @param {function_to_call}     function_to_call  Callback function to call when an event occurred. The function must accept one parameter: the current scene.
 * @param {boolean} loop 
 */
...
PP.physics.set_acceleration_x = function(obj, acc_x) {
    ...
    PP.debug.assert(typeof obj.ph_obj === "object",  "Parameter error: obj is an object but it is not a valid object.");
    PP.debug.assert(obj.has_physics, "Parameter error: obj does not have physics.");
    PP.debug.assert(typeof acc_x === "number", "Parameter error: vel_x should be a number.");
...
PP.physics.set_acceleration_y = function(obj, acc_y) {
    ...
    PP.debug.assert(typeof obj.ph_obj === "object",  "Parameter error: obj is an object but it is not a valid object.");
    PP.debug.assert(obj.has_physics, "Parameter error: obj does not have physics.");
    PP.debug.assert(typeof acc_y === "number", "Parameter error: vel_x should be a number.");
...
PP.physics.add_collider = function(s, obj1, obj2) {
    ...
    s.physics.add.collider(obj1.ph_obj, obj2.ph_obj);
}
...
PP.physics.add_collider_f = function(s, obj1, obj2, func) {
    ...
    s.physics.add.collider(obj1.ph_obj, obj2.ph_obj, (a,b)=>{func(s,a,b)});
}
...
PP.physics.add_overlap_f = function(s, obj1, obj2, func) {
    ...
    s.physics.add.overlap(obj1.ph_obj, obj2.ph_obj, (a,b)=>{func(s,a,b)});
}
...
/**
 * 
 * Set a different gravity value for this specific object
 * @param {object} obj      The object previously added to the physics engine
 * @param {float}  gravity   Gravity value
 */
PP.physics.set_gravity = function(obj, gravity) {
    PP.debug.assert(typeof obj === "object", "Parameter error: obj should be an object.");
    PP.debug.assert(typeof obj.ph_obj === "object",  "Parameter error: obj is an object but it is not a valid object.");
    PP.debug.assert(obj.has_physics, "Parameter error: obj does not have physics.");
    PP.debug.assert(typeof gravity === "number", "Parameter error: gravity should be a number.");

    obj.ph_obj.body.setGravityY(gravity);
}


// TODO:
// - Alpha per tutti i valori